...
import { party } from '../../api-services/app.classes';
...
@Component({
  ...
  styleUrl: './search-party.component.scss',
})
export class SearchPartyComponent extends Control implements OnInit {
  ...
  createParty() {
    this.api.createParty().subscribe((success) => {
      this.party = success;
      this.party.partyName = this.searchText;
    });
  }

  saveParty() {
    this.api.saveParty(this.party).subscribe((success) => {
      this.party = success;
      this.slideClose();
    });
  }
